...
Backend обязательно разбить на микросервисы. Выделяю три микросервиса: работа с получением статей из БД, работа с пользователями и работа с изменением статей. Это необходимо из-за огромного различия в нагрузке на просмотр статей и на работа с юзерами и изменением статей. Просмотр статей гораздо в несколько тысяч раз имеет больше RPS, поэтому микросервис по выгрузке статей из бд будет иметь гораздо больше нод.
